...
Again, the syntax is straightforward where `style` is preceded by the name of a CSS style property: `[style.color]`. In this example, the color style property binds to the template expression, `isHoussein()`. With the code copied to your project, try changing the name to *Houssein* through the input field or by clicking the button.
...
`[class.awesome]` binds to the specific `.awesome` class when the template expression, `isHoussein()`, is truthy.
...
In this post, I covered the basics of one-way and two-way data binding as well as showed how you can manipulate style properties and classes with a few different ways. Hope it proved useful and stay tuned for my next post on Angular 2.
